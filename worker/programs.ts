...
export interface PriceOption {
   ...
   *
   * Exact means exact, including the em dashes: QuickBooks' query language has
   * no fuzzy match, so "Grom's - 10 classes" typed with a hyphen is a different
   * item and the lookup fails. Katie should paste these rather than retype them,
   * and `/qbo/verify-items` reports any that do not resolve — which is how a
   * mismatch gets caught by us rather than by a parent mid-enrollment.
   *
